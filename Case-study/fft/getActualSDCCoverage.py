...
proc1 = createNewSubprocess(["python3", "Utils/getFilteredFailureRates.py", fi_folder_path, str(noT), ""])
while True:
...
with open(llfi_indices_file_path, "r") as f:
    for line in f.readlines():
        ...
        llfi_index_set = ""
        if len(line.strip().split(":")[1].strip()) > 0:
            llfi_index_set = str(line.strip().split(":")[1].strip())
        print("llfi_index_set: ", llfi_index_set)
        
        sdc_rate = 0.0
        proc2 = createNewSubprocess(["python3", "Utils/getFilteredFailureRates.py", fi_folder_path, str(noT), llfi_index_set])
        while True:
