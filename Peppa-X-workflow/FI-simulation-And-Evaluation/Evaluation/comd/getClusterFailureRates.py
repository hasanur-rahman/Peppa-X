...
def calculateSDC():
	...
	for i in range(1, numSubFolder+1):
...
def getOutput(lines):
    ...
    for line in lines:
        if 'Simulation Validation:' in line:
                is_started = True
        if is_started and len(line) > 0:
        	output.append(line.strip())
        if is_started and 'Final atom count' in line:
        	is_started = False
